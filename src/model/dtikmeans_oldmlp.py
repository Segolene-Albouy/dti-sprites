...
from .transformer import PrototypeTransformationNetwork as Transformer
from .tools import (
    copy_with_noise,
...
class DTIKmeans(nn.Module):
    ...
    def forward(self, x):
        ...
        return dist.mean(), distances  # , dist_min_by_sample, argmin_idx
    ...
    def transform(self, x, inverse=False):
        if inverse:
            ...
            B, C, _, _ = x.size()
            prototypes = self.prototypes.unsqueeze(1).expand(-1, B, C, -1, -1)
